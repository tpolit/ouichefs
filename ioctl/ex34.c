...
static long do_version(int ver, char *cible)
{
	...
	/*cible_f = filp_open("/share/ouichefs-master/mkfs/ouiche_dir/file",
	O_RDONLY, 0400); */
	...
	/* accés à l'inode sur disque */
	/* inode_block = (c_inode->i_ino / OUICHEFS_INODES_PER_BLOCK) + 1;
	inode_shift = c_inode->i_ino % OUICHEFS_INODES_PER_BLOCK;

	temp_bh = sb_bread(c_inode->i_sb, inode_block);
	if (!temp_bh) {
		return -EIO;
	}
	d_inode = (struct ouichefs_inode *)temp_bh->b_data;
	d_inode += inode_shift;
	current_index = le32_to_cpu(d_inode->last_index_block);
	brelse(temp_bh);*/
	...
	for (nb_prev = ver; nb_prev > 0; nb_prev--) {
		...
		/* arrivé à la 1ére version et on veut
		quand même reculer plus */
		if (block->prev == -1)
			return -1;
	...
	/* mettre à jour l'inode sur disque */
	/*temp_bh = sb_bread(c_inode->i_sb, inode_block);
	if (!temp_bh) {
		return -EIO;
	}
	d_inode = (struct ouichefs_inode *)temp_bh->b_data;
	d_inode += inode_shift;
	d_inode->index_block = cpu_to_le32(current_index);
	brelse(temp_bh);*/
...
static long do_rewind(char *cible)
{
	...
	tout en liberant tout les blocs utilisé par les
	versions entre now et last(inclus)
	*/
	struct inode *c_inode;
	struct ouichefs_inode_info *co_inode;
	struct buffer_head *temp_bh;
...
static long ul_ioctl(struct file *file, unsigned int cmd,
	unsigned long arg)
{
	struct parameters *p;
	...
	case VERSION:
		...
		p = (struct parameters *)
			kmalloc(sizeof(struct parameters) +
			size_filename, GFP_KERNEL);
	...
	case REWIND:
		...
		pr = (struct parameters_rewind *)
			kmalloc(sizeof(struct parameters_rewind) +
				size_filename, GFP_KERNEL);
...
static int __init current_view_init(void)
{
	...
	file_op.unlocked_ioctl = ul_ioctl;
